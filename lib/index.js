...
var SyncHub = class {
	...
	*/
	transcript(machineName, sessionId, page = { limit: TRANSCRIPT_WINDOW }) {
		...
		const originHasOlder = held !== void 0 && session.originFirstSeq >= 0 && held > session.originFirstSeq;
...
var SessionSyncService = class SessionSyncService {
		...
		this.link?.publishIndex({
			...
			sessions: rows.filter((row) => row.synced).map((row) => {
				...
				return {
					...
					...firstSeq === void 0 || firstSeq < 0 ? {} : { firstSeq }
				};
			})
		});
	}
	...
	*/
	...
	absorb(handle, frame) {
		...
		if ((frameType === "snapshot" || frameType === "opened") && typeof carrier["cursor"] === "number") handle.cursor = carrier["cursor"];
