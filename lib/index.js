		...
		for (const machine of machines) for (const session of machine.sessions) {
			...
			try {
				...
				if (result.ok && result.written > 0) {
					...
					report.advanced += 1;
				} else if (result.ok) this.waits.delete(session.sessionId);
				else if (result.wait === true) this.waits.set(session.sessionId, {
					at: Date.now(),
					reason: result.reason ?? "the mirror is not ready to be appended"
				});
				else await this.ledger.stop(session.sessionId, result.reason ?? "the copy stopped tracking the mirror");
			} finally {
