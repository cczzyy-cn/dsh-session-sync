...
var SyncHub = class {
	...
	*/
	reportGap(record, session) {
		...
		if (asked === void 0) this.logger?.warn(`dsh-session-sync: mirror for "${session.sessionId}" on "${record.machineName}" is missing ${String(missing)} event(s) between seq ${String(lowest)} and ${String(session.maxSeq)}; asked that machine to replay the Session`);
