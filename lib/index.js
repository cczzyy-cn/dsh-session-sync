...
var SyncHub = class {
	...
	*/
	submitCommand(machineName, sessionId, text, from) {
		...
		if (record.origin === void 0) record.pending.push(command);
...
var SessionSyncService = class SessionSyncService {
	...
	*/
	async localSessions() {
		...
		return items.filter((item) => item.parentSessionId === void 0 && item.origin !== "subagent").map((item) => this.row(item)).sort((left, right) => right.updatedAt - left.updatedAt);
	}
	/**
	...
	*/
	...
	startFollow(sessionId) {
		...
		(async () => {
			try {
				...
				if (!handle.abort.signal.aborted) this.ctx.logger.warn(`dsh-session-sync: follow for "${sessionId}" ended: ${describe$1(error)}`);
			} finally {
	...
	async runCommand(command) {
		...
		try {
			...
			this.ctx.logger.warn(`dsh-session-sync: takeover prompt failed: ${describe$1(error)}`);
		}
	}
