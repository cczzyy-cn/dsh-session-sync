...
var SessionSyncService = class SessionSyncService {
	...
	*/
	async copyDiverged(persistence, machineName, sessionId, stored) {
		if (stored === 0) return false;
		const tail = this.hub.transcript(machineName, sessionId, {
			limit: 8,
			before: stored
		})?.events ?? [];
		if (tail.length === 0) return false;
		return await logAgreesWithMirror(persistence, sessionId, tail, stored).catch(() => void 0) === false;
	}
	/**
	...
	*/
	async syncMaterialized() {
		...
		for (const machine of machines) for (const session of machine.sessions) {
			...
			const heldNow = await storedEventCount(persistence, session.sessionId).catch(() => void 0);
			if (heldNow !== void 0 && heldNow > needs && await this.copyDiverged(persistence, machine.machineName, session.sessionId, heldNow)) {
			...
			try {
				...
				const result = await catchUpSession(persistence, session.sessionId, transcript.events);
		...
		for (const machine of machines) for (const session of machine.sessions) {
			...
			if (start === "adopt") {
				...
				if (count !== void 0 && await this.copyDiverged(persistence, machine.machineName, session.sessionId, count)) await this.ledger.stop(session.sessionId, DIVERGED_REASON);
				report.adopted += 1;
				this.broadcastState();
				return report;
			}
