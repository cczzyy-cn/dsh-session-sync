...
var SyncHub = class {
	...
	*/
	reportGap(machineName, session) {
		const lowest = session.events[0]?.seq;
		if (lowest === void 0) return;
		...
		this.logger?.warn(`dsh-session-sync: mirror for "${session.sessionId}" on "${machineName}" is missing ${String(missing)} event(s) between seq ${String(lowest)} and ${String(session.maxSeq)}; only re-publishing the Session clears it`);
	}
	/**
...
function summary(session) {
	const lowest = session.events[0]?.seq;
	const missing = lowest === void 0 ? 0 : Math.max(0, session.maxSeq - lowest + 1 - session.seqs.size);
	return {
		sessionId: session.sessionId,
		title: session.title,
		updatedAt: session.updatedAt,
	...
	*/
	publishFrames(sessionId, events) {
		...
		if (this.framesQueued >= FRAME_QUEUE_WARN && !this.frameQueueWarned) {
			...
			this.options.logger.warn(`dsh-session-sync: ${String(this.framesQueued)} frame batches queued; the link is behind`);
		}
		this.frameQueue = this.frameQueue.then(async () => {
			if (!await this.post("/frames", body)) this.options.logger.warn(`dsh-session-sync: ${String(events.length)} durable event(s) for "${sessionId}" were not accepted by the server; re-publish the Session to rebuild its mirror`);
			this.framesQueued -= 1;
