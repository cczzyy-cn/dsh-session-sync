	...
	*/
	async syncMaterialized() {
		...
		for (const machine of machines) for (const session of machine.sessions) {
			...
			const transcript = this.hub.transcript(machine.machineName, session.sessionId, { limit: 1e5 });
			...
			if (result.ok && result.written > 0) {
				...
				report.advanced += 1;
			} else if (!result.ok) await this.ledger.stop(session.sessionId, result.reason ?? "the copy stopped tracking the mirror");
		}
		for (const machine of machines) for (const session of machine.sessions) {
			if (this.ledger.owns(session.sessionId)) continue;
			if (this.hub.transcript(machine.machineName, session.sessionId, { limit: 1 }) === void 0) continue;
			const stored = await persistence.stat(session.sessionId).catch(() => void 0);
