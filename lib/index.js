...
var SyncHub = class {
	...
	*/
	reportGap(record, session) {
		...
		for (const hole of holes) record.origin?.older(session.sessionId, hole.from - 1, HOLE_PAGE_MESSAGES);
...
async function materializeSession(persistence, workspaces, input) {
	...
	if (expected < input.events.at(-1).seq + 1) return {
		ok: false,
		written: 0,
		skipped,
		archived: false,
		reason: `the mirror's run stops at seq ${String(expected - 1)}; the rest is not contiguous, so nothing was written — the Session needs its gaps filled first`
	};
	let handle;
	try {
...
var SessionSyncService = class SessionSyncService {
	...
	*/
	async materialize(machineName, sessionId) {
		...
		try {
			...
			return await materializeSession(this.ctx.get("sessionPersistence"), this.ctx.get("workspaceRegistry"), {
				sessionId,
	...
	*/
	async pullOlder(sessionId, throughSeq, maxMessages) {
		...
		if (skip !== void 0 || handle === void 0) return;
		const now = Date.now();
