...
**线上现状**：`v0.4.3` 已部署、`active`；线上镜像此刻**没有洞**（`296 条 / 4570..4865 / holes=0`），所以这条修复在线上**仍缺观测点**——已挂 15 分钟监控（每 20 秒记 `missingEvents` 与空洞数），等它自然遇到一次丢批。判据：`missingEvents` 由正数回落到 0，且源站 `state.page` 出现 `beforeSeq = 洞的第一条`。
