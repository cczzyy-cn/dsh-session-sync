...
| 版本 | 本地 = 远端 = `fc8993f`（本轮长会话回填修复） |
| 服务器 | `210.16.120.228` · Ubuntu 24.04 · **DSH `0.1.7-rc.2`（npm `next` 通道，未打补丁）** · 插件 **`13b7aa2`**（`fc8993f` 与它只差文档） · unit `dsh-web.service` · active |
| 本机 | DSH 源码运行（checkout = `dsh-v0.1.7-rc.1` 标签），运行时用 rc.1 源码 · **跑的还是重启前的旧插件**（`13b7aa2` 的切批要重启本机才生效） |
...
**2026-09-25 服务器更新**：插件 → `13b7aa2`（长会话回填修复：按字节切批 + 具名 413），`pnpm update` 的 lock 与安装产物都已核对，`systemctl restart dsh-web` 后 active、3080/8791 在听。**本机源站尚未重启**，所以线上那条长会话要等本机重启后才会自己回填。
...
| 4 | **部署到服务器并线上验证** | 🔶 服务器 ✅ / 本机待重启 | 服务器：插件已 `pnpm update` 到 **`13b7aa2`**（lock 的 tar.gz 与安装产物都用 `FRAMES_BODY_BYTES`/`stripByteOrderMark` 标记核对过），`dsh-web` restart 后 active、3080/8791 都在听。**本机源站仍未重启**（用户选择自己重启）⇒ 线上长会话回填要等本机带上切批逻辑 |
