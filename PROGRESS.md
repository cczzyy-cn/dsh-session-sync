...
| 版本 | **`0.4.1`**（tag `v0.4.1` → `342d914`）· 本地 = 远端 |
| 服务器 | `210.16.120.228` · Ubuntu 24.04 · **DSH `0.1.7-rc.2`（npm `next` 通道，未打补丁）** · 插件 **`v0.4.1`**（依赖钉 tag，lock → `342d914`）· unit `dsh-web.service` · active |
...
| # | 任务 | 状态 | 说明 |
| --- | --- | --- | --- |
| 1 | **长会话回填**（镜像只有尾部窗口时，翻到 seq 0 再物化） | ✅ **通了**（2026-09-25，见下） | 两个根因先后修掉：**一批装不下**（`ff98129`）与**每页边界丢一条**（`3ee53ff`）。受控实测把镜像从 3158 走到 0：**3478 条、零空洞**；物化 `written 3478 / skipped 0 / archived true` |
...
### 中段空洞：修好了，但线上暂时没有洞可修（`0.4.1`，2026-09-25 晚）
...
- `reportGap` 对每个洞发一次 `older(sessionId, hole.from - 1, 500)`。`throughSeq` 按契约是**闭**上界，所以"洞上沿那条"正好让源站把自己日志里**从洞到该处**的那一页读回来，作为普通持久事件到达；
- 重放 ask 与洞页 ask **一起发**，因为它们修的不是同一件事（重放补"窗口顶部与落后"，洞页补"重放够不到的下面"），并且**共用同一个重试下限**——一台一直失败的机器，每个洞每轮只被问一次，而不是每来一批就问一次。

**测试** `tests/hole-repair.spec.ts`（5 条）：ask 报的正是洞上沿；两个洞都会被问；洞没补上时 sweep 会再问；补上之后**一条都不问**；下限仍然约束重试。为了测"第二次 ask"，把 hub 的两个重试下限做成可注入（`timings`）——这类时间常量本来就不该埋在逻辑里。

**线上现状（诚实记录）**：`v0.4.1` 已部署、`active`，但**当前镜像 `missing: 0`，没有洞**，所以这条修复在线上**还没有观测点**。等它真的遇到一次空洞（丢批/重放留下的），判据是：`missingEvents` 在若干秒内回落到 0，且源站 `state.page` 会出现一条 `beforeSeq = 洞上沿` 的读数。

> 教训：**同一个不变量（窗口连续）有两个不同的破坏方式**（落后 / 中段缺失），而修它们的手段也不同（重放 / 定点读页）。只实现一个，另一个就会一直"修不了"——而它看起来像是同一个问题。
