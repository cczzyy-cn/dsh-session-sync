...
| 版本 | 本地 = 远端 = `b2a7811`（长会话回填：切批 + 分页边界） |
| 服务器 | `210.16.120.228` · Ubuntu 24.04 · **DSH `0.1.7-rc.2`（npm `next` 通道，未打补丁）** · 插件 **`b2a7811`** · unit `dsh-web.service` · active |
| 本机 | DSH 源码运行（checkout = `dsh-v0.1.7-rc.1` 标签），运行时用 rc.1 源码 · 16:26 重启过，跑的是 `13b7aa2`，**缺边界修正**（`pullOlder` 在源站侧）⇒ 要再重启一次本机 |
