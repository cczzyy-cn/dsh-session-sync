...
| 版本 | **`0.4.2`**（tag `v0.4.2` → `6875020`）· 本地 = 远端 |
| 服务器 | `210.16.120.228` · Ubuntu 24.04 · **DSH `0.1.7-rc.2`（npm `next` 通道，未打补丁）** · 插件 **`v0.4.2`**（依赖钉 tag，lock → `6875020`）· unit `dsh-web.service` · active |
...
### 中段空洞：`0.4.1` 写错了上界，`0.4.2` 修好（2026-09-25 晚）
...
**仍未查清（如实记录，不当成没问题）**：那条端到端测试**不再断言"镜像最终完整"**，因为实测到：源站读对了页、缓冲了 151 条，而**镜像仍差一条**——原因是 `resync` 在页读在途时拆掉了 follow，而那一页正缓冲在它的 handle 里。这是"第二个缺陷"还是"两套修复机制相撞的代价"，**尚未定论**。测试里写明了这个边界，PROGRESS 记着这个问号，不拿绿灯掩盖。

**线上现状**：`v0.4.2` 已部署、`active`，但**当前镜像 `missing: 0`，没有洞**，所以这条修复在线上**还没有观测点**。等它真遇到一次空洞，判据是：`missingEvents` 若干秒内回落到 0，且源站 `state.page` 出现一条 `beforeSeq = 洞的第一条` 的读数。
