...
### 完成度检查：查到三个缺口，两个当场补掉（v0.5.8，2026-09-25）

用户要求"检查插件完成度"。检查不是凭记忆做的，而是重新读了仓库、测试、标签、设置页与线上状态，结论与处理：

| 缺口 | 性质 | 处置 |
| --- | --- | --- |
| **`materialize` 开关没有设置页控件** | 我引入的：协议里有（`SyncConfig.materialize`）、Host 也认（`service.patch`），但 `ConfigSection.tsx` 一次都没提、`locales.ts` 没有文案——而 README 却写着"Leave **写成真会话** on" | **已补**：设置页 `会话同步` 组里加了开关（随草稿一起保存，与 `作为服务器` 同组）+ 一个 **`已写成真会话`** 列表（条数 + `已停更` 标记，重建步骤在 tooltip 里） |
| **PROGRESS §0.5 任务清单过时** | 2/3/4/5 四条做完并**线上验证过**，表里还写着"未开始"；4 还写着"本机待重启"而本机早已跑新构建 | **已改**：按实际重写，并写明与原始目标的一处偏离（不归档、改门禁） |
| **`docs/host-side-session-plan.md` 仍推荐已被证伪的方案** | 它的 §2 写着"物化 + **归档**（推荐）"、§9 以 `archiveSession` 收尾，而实测证明归档会让会话**打不开** | **已改**：文首加了 ⚠️ 更正块（不归档 + 门禁 + 台账；写入顺序的收尾换成"记台账"与"追加前比对"），正文保留为当时的推理记录 |
| **客户端半边零自动化测试**（12 个测试全在 Host 侧） | 根因：那半边 import `react` 与 shipped UI 包，本仓库加载不了 | **已修一半**：把两条**决策**抽到无依赖的 `src/client/routing.ts`（`freshIds` / `rowTarget`），`SyncPanel` 只负责渲染；新增 `tests/client-routing.spec.ts`。测试 **63 → 71** |

**这一轮同时是自查**：我核对了新写的设置页用到的**每一个 CSS 类**是否真的存在于 `sync.module.css`（`css.fieldRow`/`fieldText` 是照邻近形状写的，构建**查不出来**）——全部存在，没有隐形失样式。

**部署与验证**：本轮**只改客户端半边**，所以不需要重启 Host。
- 服务器：profile 钉 `#v0.5.8` + `pnpm install`，lock → `082ee2a`；产物标记在（`写成真会话` ×2、`已写成真会话` ×1；Host 半边的台账标记仍在，未变）；`dsh-web` 全程 active。
- 浏览器复核：设置 → `会话同步` 里 **`作为服务器` 开、`写成真会话` 开**，说明文字正确（截图）。
- 本机（源站）：仍是旧 main；因为它钉的是裸 `github:`，`pnpm --dir ~/.dsh/profiles/web update dsh-session-sync` + 刷新页面即可拿到新设置页（同样不需要重启）。

**仍未验／仍未做**（诚实清单）：`已写成真会话` 那块列表**没有目视确认**（它用的是同一批已验证存在的类，但滚动没驱动起来）；官方侧的**跳转**（时间线条可点中、账本没跟着跳）与轨迹搜索；非中文界面；官方页上 `ui-deliverables` 的 `/api/changes.summary` 404（见上一条）。
