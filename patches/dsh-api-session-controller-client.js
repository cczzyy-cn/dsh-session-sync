window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region lib/types/client/sessions/adopted.js
		/**
		...
		*/
		var AdoptedSessionHandle = class {
			sessionId;
			session;
			drop;
			...
			*/
			constructor(sessionId, session, drop) {
			...
			*/
			live(attempt) {
				...
				if (live !== void 0 && (!attempt.text.startsWith(live.text) || live.turn !== attempt.turn)) {
			...
			*/
			...
			/** Close one attempt's frame run, when it is the one this client opened. */
				...
				this.frame({
					...
					revision: live.revision,
					index: live.nextIndex,
					outcome: outcome.kind === "abandoned" ? { kind: "abandoned" } : {
						kind: "committed",
		...
		var Session = class {
			...
			*/
			constructor(sessionId, remote, options = {}) {
				...
				if (this.mirror !== void 0) this.openState = "open";
			...
			*/
			async prompt(content, mode, signal, requestId) {
				...
				this.notifier.markDirty();
				let result;
				if (this.mirror !== void 0) result = this.mirror.prompt === void 0 ? {
					ok: false,
			...
			*/
			async cancel() {
				...
				const result = mirror !== void 0 ? mirror.cancel === void 0 ? {
					ok: false,
					error: new RemoteError("gateway/bad-request", "this Session is a mirror and cannot be stopped here", {})
				} : await mirror.cancel() : address !== void 0 ? await this.remote.subagents.interruptByParent(address.childSessionId, address.parentSessionId, "continuable") : await this.remote.session.cancel({ sessionId: this.sessionId });
				if (!result.ok) {
					this.promptError = {
						op: "stop",
						error: result.error
				...
				this.listInflight = (async () => {
					try {
						...
						if (result.ok) {
							...
							this.summaries = [...this.adopted.values()].reduce((current, summary) => applyMutation(current, {
								kind: "upsert",
								summary
							}), summaries);
		...
		var ClientSessions = class {
			...
			*/
			adopt(source) {
				...
				if (this.scopes.has(id) || this.list.getSnapshot().byId[id] !== void 0) throw new Error(`Session "${id}" is already active in this Controller`);
				...
				const session = this.manager.adopt(id, {
					...
					summary: {
						...
						blank: false,
						...source.summary?.cwd === void 0 ? {} : { cwd: source.summary.cwd },
						...title === void 0 ? {} : { projections: {
							kind: "cached",
							asOfSeq: -1,
							values: { title }
						} }
